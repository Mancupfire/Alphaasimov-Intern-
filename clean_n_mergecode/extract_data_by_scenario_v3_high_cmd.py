...
STEP = 0.1
AP_MODE = 2
EXTRACT_DIR = '/media/asimovsimpc/bulldog1/extracted_data'
CAMERA_NAME = '/front'
...
parser.add_argument('--bag', dest='bag_file', default='sample.bag', type=str, help='Name of the rosbag file')
parser.add_argument('--rate', dest='rate', default=1, type=int, help='multiply rosbag rate record')
parser.add_argument('--fps', dest='fps', default=20, type=float, help='frame rate of rosbag record')
parser.add_argument('--min_lin_vel', dest='min_lin_vel', default=LIN_VEL_LIMIT_DEFAULT[0], type=float, help='Min linear velocity')
parser.add_argument('--max_lin_vel', dest='max_lin_vel', default=LIN_VEL_LIMIT_DEFAULT[1], type=float, help='Max linear velocity')
parser.add_argument('--dir', dest='temp_extract_dir', default=EXTRACT_DIR, type=str, help='Extracted directory.')
parser.add_argument('--cam_name', dest='cam_name', default=CAMERA_NAME, type=str, help='camera name for writing to csv file')
parser.add_argument('--cam_topic', dest='cam_topic', default=CAMERA_TOPIC, type=str, help='camera topic in bag file')
...
class LogData():
    def __init__(self, camera_name, topic_name, path_dir):
        ...
        self.rate = rospy.Rate(int(args.rate * args.fps))
        
        # self.rate = rospy.Rate(40)

        self.threash_rate = args.fps * args.rate

        self.lin_vel_limit = [args.min_lin_vel, args.max_lin_vel+LIN_VEL_LIMIT_TOLERANCE]
    ...
    def odomCallback(self, msg):
        ...
        self.linear_vel = self.linear_vel_feedback
        self.angular_vel = self.angular_vel_feedback
    ...
    def remoteCmdCallback(self, data):
        # ROS Subscriber remote_control topic callback function:
        self.enable_collect_data_rc = data.remote_control.enable_collect_data
        ...
        self.dir_high_cmd = data.remote_control.direct_high_cmd
    ...
    def saveFrames(self):
                ...
                try:
                    ...
                    if self.img_callback_flag == True:
                        ...
                        if (self.linear_vel > self.lin_vel_limit[0]) and (self.linear_vel < self.lin_vel_limit[1]):
                            ...
                            # write to file CSV
                            # writer.writerow([fullNameImage, self.angular_vel, self.linear_vel, self.dir_high_cmd, self.gps_msg.latitude, self.gps_msg.longitude, self.gps_error,
                            #                  self.yaw, self.imu_msg.orientation.x, self.imu_msg.orientation.y, self.imu_msg.orientation.z, self.imu_msg.orientation.w,
        ...
        with open(os.path.join(self.logDir, self.log_filename), 'w') as logFile:
            writer=csv.writer(logFile, delimiter=',',lineterminator='\n',)
            writer.writerow(["Total frames", "Correct frame", "Correct ratio"])
            writer.writerow([total_sample, correct_sample, f'{(correct_sample/total_sample * 100):.2f}'])

        # with open(os.path.join(self.logDir, self.log_filename), 'w') as logFile:
        #     logFile.write(f'Total frames in rosbag: {total_sample}\n')
        #     logFile.write(f'Correct frames in rosbag: {correct_sample}\n')
        #     logFile.write(f'Percentage correct frames in rosbag: {(correct_sample/total_sample * 100):0.2f}%\n') 
...
if __name__ == '__main__':
    a = LogData(args.cam_name, args.cam_topic, args.temp_extract_dir)
    a.main()