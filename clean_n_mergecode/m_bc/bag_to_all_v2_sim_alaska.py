...
SAVE_DIR = '/media/asimovsimpc/bulldog/aa-data/extracted_data/simulation/alaska'
BAGFILE = '/media/asimovsimpc/bulldog/aa-data/simulation/alaska/twoparks/mix/2024_01_05/2024_01_05_16_38_02.bag'
...
parser.add_argument('--data', dest='data_type', default='sim', type=str, help='Data type: umtn or bc or sim')
...
def write_high_cmd_v2( writer, msg, date_time):
    dir_high_cmd = msg.high_cmd.data
    sensor_data = [date_time, dir_high_cmd]
...
def read_scenario(bag, dataType):
    ...
    scenario_dict = {}
    if "sim" in dataType:
        return {1: {"scenario": "mix", "start_time": 0, "end_time": 100000000000000},}
...
def get_logDir(scenario, args):
    ...
    folder_name = args.bag_file.split('/')[-4]
    ...
    folder_name = args.bag_file.split('/')[-2][:10]
...
def bag2csv(bag, dataType, scenario_dict, args):
    ...
    elif 'sim' in dataType:
        listOfTopics = ["/imu/data", "/dir_high_cmd", "/odom", "/ublox/fix",]
        csv_filename = ["imu", "high_cmd", "speed", "gps"]
        function_dict = {
            "/imu/data": write_imu,
            "/dir_high_cmd": write_high_cmd_v2,
            "/odom": write_speed,
            "/ublox/fix": write_gps,
        }
    ...
    for id, topic_name in enumerate(listOfTopics):
        ...
        for key in scenario_dict.keys():
            if key != 0:
                log_dir = get_logDir(scenario_dict[key], args)
                scenario_dict[key][csv_filename[id]] = f"{log_dir}/{csv_filename[id]}.csv"
        ...
        for seq_id, bag_data in enumerate(extracted_data): 
            ...
            if old_time != new_time:  
                old_time = new_time
                try:
                    # if old_time > scenario_dict[current_scenario_dict_seq]["end_time"] or id == 4:
                    if 1==1:
                        # print("haha")
                        current_scenario_dict_seq += 1
                        try:
                            open(scenario_dict[current_scenario_dict_seq][csv_filename[id]], 'w+')
                            file.close()
                        except:
                            pass
                        file = open(scenario_dict[current_scenario_dict_seq][csv_filename[id]], 'w+')
    ...
    print(f"Current scenario is {(scenario_dict[1]['scenario'])}")
...
def bag2png(bag, dataType, scenario_dict, args):
    ...
    for topic in list_of_topics:
        ...
        imgDir = get_logDir(scenario_dict[1], args)
...
if __name__ == '__main__':
    ...
    for key in scenario_dict.keys():
        if key != 0:
            logDir = get_logDir(scenario_dict[key], args)
