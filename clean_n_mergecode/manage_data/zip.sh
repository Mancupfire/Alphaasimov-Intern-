...
python name_modified_select.py

mkdir -p $packpath
echo -e "\e[36m$packpath\e[0m created"

# Prompt for the password before the loop
#read -p "Enter password for zip files: " zip_password
zip_password='nEoHyCmjg0k2LpEu2qkDOU8EUH3Bn39feOqdan0r'
echo $zip_password
echo
...
while IFS= read -r record_dir; do
    ...
    common_prefix=$(echo "$record_dir" | sed 's/.*extracted_data\///; s/\/[^/]*$//')
   ...
   # Copy the contents into the temporary directory (CORRECTED)
    ...
    # Zip the temporary directory, preserving the structure using 7zip with password protection
    7z a -tzip "$packpath/$record_name.zip" "$common_prefix" -p"$zip_password"
