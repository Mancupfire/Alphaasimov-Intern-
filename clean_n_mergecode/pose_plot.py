...
import argparse
import re
from auto_data_checking import discontinuity_detector
...
class AutoDetector:
    def __init__(self, detector: discontinuity_detector.InterpolationBasedDetector):
    ...
    def auto_check_data(self, data_dir: str, log_path: str = 'autodetected_wrong_data.log') -> tuple:
        ...
        """
    #    with open(log_path, 'a') as f:      #Original HA
    #        f.close()
    #
    #    files_to_check = []
    ...
    #                files_to_check.append(os.path.join(root, dir, 'final.csv'))
        ...
        files_to_check = []
        for root, dirs, files in os.walk(data_dir): # Recursively walk through the directory
            for file in files:
                if file == 'final.csv':
        ...
        for csv_path in tqdm(files_to_check, desc='Checking files', total=numb_files, unit='files'):
            ...
            csv_path = os.path.dirname(csv_path) # remove 'final.csv' from the path
...
class HumanVerifier:
    ...
    def verify_single_record(self, data_dir: str, comment: str = None) -> None:
        ...
        #if not extracted_data_utm:f
        #raise ValueError(f"Data hasn't been (or cannot be) extracted in {data_dir}")

        intervals = self.extract_interval_from_comment(comment)
        displayed_intervals = [
            [max(i[0]-60, 0), min(i[1]+60, len(extracted_data_utm))] for i in intervals 
        ]

        while displayed_intervals:
            ...
            for i, ax in enumerate(axes.flat):
                ...
                # Create a new axis for the fitting curve, sharing the x-axis with the existing axis
                ax2 = ax.twinx()

                # Plot the fitting curve for the first interval in the displayed intervals on the new axis
                self.detector.plot_fitting_curve(trajectory, index=0, ax=ax2)
...
if __name__ == '__main__':
    ...
    autocheck_parent_dir= "/media/asimovsimpc/bulldog/aa-data/extracted_data/bulldog/umtn-tele-joystick/ocp/mix/2024_07_27"
    detector = discontinuity_detector.InterpolationBasedDetector(
        window_size=30, # old: 30
