...
def extract_data_and_convert_to_useful_info(file_path, silent=False):
    ...
    with open(file_path, 'r') as csvfile:
        ...
        for row in reader:
            ...
            fusiondata_keys = ['pose_pos_x', 'pose_pos_y', 'pose_pos_z', 'pose_ori_x', 'pose_ori_y', 'pose_ori_z',
                               'pose_ori_w']
...
class AutoDetector:
    def __init__(self, detector: discontinuity_detector_fit_point.InterpolationBasedDetector):
    ...
    def auto_check_data(self, data_dir: str, log_path: str = 'autodetected_wrong_data.log') -> tuple:
        ...
        for csv_path in tqdm(files_to_check, desc='Checking files', total=numb_files, unit='files'):
            ...
            if not data:
                ...
                distance_anomalies = self.detector.detect_distance_anomalies(data)

                if np.sum(discontinuities) < 30 and not np.any(distance_anomalies):
                    continue

                # Log distance anomalies to a separate file
                if np.any(distance_anomalies):
                    with open('distance_anomaly.log', 'a') as f:
                        anomaly_indices = np.where(distance_anomalies)[0]
                        f.write(f"{csv_path}\tDistance anomalies detected at indices: {anomaly_indices}\n")

                errors = self.detector.curve_fitting_errors(trajectory)
                above_threshold = np.where(errors > self.detector.threshold)[0]
...
class HumanVerifier:
    ...
    def extract_interval_from_comment(self, comment: str):
        """
        Extract the intervals, RMSE values, and coefficients from the comment string.

        Args:
            comment (str): The comment string containing information about discontinuities, 
                        RMSE values, and coefficients.

        Returns:
            tuple: A tuple containing three lists:
                - intervals (list): A list of tuples representing the start and end indices of 
                                    discontinuity intervals.
                - rmse_values (list): A list of RMSE values corresponding to each discontinuity.
                - coefficients (list): A list of coefficient arrays for the fitted curves at 
                                    each discontinuity.
        """
        ...
        if rmse_values and coefficients_match:
            try:  # Add a try-except block to handle potential ValueErrors
            ...
            rmse_values = []  # Assign an empty list if no RMSE values are found
            coefficients = []
    ...
    def verify_single_record(self, data_dir: str, comment: str = None) -> None:
      ...
      visualization_choice = input("Choose visualization: (f)itted curve or (g)aps? ").lower()
      ...
      if visualization_choice == 'f':  # Fitted curve
          ...
          plt.tight_layout()
          plt.show()
      elif visualization_choice == 'g':  # Gaps
          discontinuities = self.detector.detect_discontinuities(trajectory)
          distance_anomalies = self.detector.detect_distance_anomalies(trajectory)

          # Pad distance_anomalies to match the length of discontinuities
          distance_anomalies = np.append(distance_anomalies, False)

          # Plot trajectory with gaps highlighted
          plt.figure()

          # Plot the normal trajectory points in blue
          normal_indices = np.where(np.logical_not(np.logical_or(discontinuities, distance_anomalies)))[0]
          plt.scatter(trajectory[normal_indices, 0], trajectory[normal_indices, 1], c='blue', s=1.5,
                      label='Normal Points')

          # Highlight detected gaps in red
          gap_indices = np.where(discontinuities == 1)[0]
          plt.scatter(trajectory[gap_indices, 0], trajectory[gap_indices, 1], c='red', s=3,
                      label='Detected Gaps (Curve Fitting)')

          # Highlight distance anomalies in purple and connect them with purple lines
          anomaly_indices = np.where(distance_anomalies)[0]
          plt.scatter(trajectory[anomaly_indices, 0], trajectory[anomaly_indices, 1], c='purple', s=3,
                      label='Distance Anomalies')
          for i in anomaly_indices:
              plt.plot(trajectory[i:i + 2, 0], trajectory[i:i + 2, 1], color='purple', linewidth=0.5, linestyle='--',
                       zorder=0)

          # Connect the normal points with lines to visualize the path
          plt.plot(trajectory[normal_indices, 0], trajectory[normal_indices, 1], color='gray', linewidth=0.5,
                   linestyle='--', zorder=0)

          plt.xlabel('Transformed X Coordinate')
          plt.ylabel('Transformed Y Coordinate')
          plt.title('Trajectory with Detected Gaps')
          plt.legend()
          plt.grid(True)
          ...
          print("Invalid choice. Please enter 'f' or 'g'.")
...
if __name__ == '__main__':
    ...
    # Load configuration from pose_plot_config.json
    with open('pose_plot_config.json') as f:
        config = json.load(f)
    autocheck_parent_dir = config['path']
    detector = discontinuity_detector_fit_point.InterpolationBasedDetector(
        window_size=60,  # old: 30
        threshold=0.01,  # old: 0.3 (meter)
        step_size=2,
        order=4  # old: 4 w
    )
