    ...
    def verify_single_record(self, data_dir: str, comment: str = None) -> None:
        ...
        while displayed_intervals:
            ...
            for i in range(4):
                ...
                # Plot the fitting curve for the first interval in the displayed intervals
                self.detector.plot_fitting_curve(trajectory, index=0)  # Plot at the beginning of the interval
...
if __name__ == '__main__':
    ...
    autocheck_parent_dir = "/media/asimovsimpc/bulldog/aa-data/extracted_data/bulldog/umtn-tele-joystick/pnk/2.2"
    detector = discontinuity_detector.InterpolationBasedDetector(
        window_size=60, # old: 30
        threshold=0.001, # old: 0.3
        step_size=1,
        order=3 # old: 4 
    )
    ...
    if args.autocheck:
        ...
        with open('pose_plot_clone_config.json') as f:
            config = json.load(f)
            path = config['path']
