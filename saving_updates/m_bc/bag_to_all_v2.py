...
def bag2png(bag, dataType, scenario_dict, args):
    ...
    elif dataType == 's-umtn-old':
        list_of_topics = ["/zed2i/zed_node/rgb/image_rect_color/compressed", "/routed_map/compressed"]
    elif dataType == 'bc':
        list_of_topics = ["/camera_front", "/camera_left", "/camera_right",
    ...
    for topic in list_of_topics:
        ...
        # if not os.path.exists(imgDir):
            # os.mkdir(imgDir)
        if topic_name == 'front' or 'zed' in topic_name:
        ...
        for id, b in tqdm(enumerate(image_topic), desc = f"Saving image {topic_name}"):    
            ...
            if ('front' in topic or 'zed' in topic):
