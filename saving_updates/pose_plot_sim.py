...
import argparse
import re
from auto_data_checking import discontinuity_detector_plus
...
def extract_data_and_convert_to_useful_info(file_path, silent=False):
    ...
    with open(file_path, 'r') as csvfile:
        ...
        for row in reader:
            ...
            if all(key in row for key in fusiondata_keys):
...
def estimate_path_from_yaw(starting_point, data):
    """
    Estimate the robot's path given starting point, yaw angles, velocity, and time.
    """
    x, y = starting_point
    path = [[x, y]]
...
def plot_transformed_coordinates(data):
    ...
    # Estimate angular velocity path
    anl_vel_path = estimate_path_from_yaw((transformed_x[0], transformed_y[0]), data)

    plt.scatter(
        [point[0] for point in anl_vel_path],
        [point[1] for point in anl_vel_path],
        label='Angular Velocity Path',
        s=1,
        color='orange'
    )
...
class AutoDetector:
    ...
    def auto_check_data(self, data_dir, log_path='autodetected_wrong_data.log'):
        """
        Auto-check data to detect discontinuities and log errors.
        """
...
if __name__ == '__main__':
    ...
    if args.autocheck:
        detector = discontinuity_detector_plus.InterpolationBasedDetector(
            window_size=60,
            threshold=0.005,
            step_size=1,
            order=3
